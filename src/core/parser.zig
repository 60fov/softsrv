...
pub const WavefrontObj = struct {
    // techinally should be 4, xyz[w]
    vertex_list: [][3]f32,

    pub fn lexBuffer(allocator: std.mem.Allocator, buf: []const u8) ![]WavefrontObjLexicalToken {
...
test {
    ...
    const token_list = try WavefrontObj.lexBuffer(allocator, file_buffer);
    defer allocator.free(token_list);
    var obj = try WavefrontObj.parseLexicalTokenList(allocator, token_list);
